...
# Format the 8_URscript_newmovel in a similar way as before to insert 
#socket_strings between the moves so we can create markers that are used in the 
#monitoring process
# Extract the number of moves to create a counter 
# Send the script to the robot controller 
# Monitor the joint speeds with the same logic we monitored at W2 script
# Write a file with the robot speed  (10_Joint_Speeds.txt)
# Calculate the linear speeds as befored
# write the linear speeds in a file (11_Linear_speeds_2.txt) 

# Setting up logging
logging.basicConfig(level=logging.DEBUG, format='%(asctime)s - %(levelname)s - %(message)s')

# Function to send a .txt file (08_URscript_newmovel) to the robot controller
...
def main():
    try:
        ...
        W1_Format_TCP.format_urscript(file_path_unf, output_file_path_1f, ip_address, message_prefix="Move")
